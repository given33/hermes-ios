...
export function observeChatDelivery(next: Message[], previous: readonly Message[], now = Date.now()): Message[] {
  ...
  return next.map((message) => {
    ...
    const hasOutput = Boolean((!message.remotePhase || message.firstTokenAt) && message.content.trim() || message.activities?.some((activity) => (
    ...
    const completedObservedAt = terminal
      ? prior?.completedObservedAt || message.completedObservedAt || now : undefined;
