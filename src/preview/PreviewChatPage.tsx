...
export function ChatPreviewPage({
  ...
  const [attachmentActionsOpen, setAttachmentActionsOpen] = useState(false);
  ...
  return (
    ...
    >
      ...
      <IOSActionSheet
        actions={[
          { id: 'library', title: isChinese ? '照片图库' : 'Photo Library' },
          { id: 'camera', title: isChinese ? '拍照' : 'Take Photo' },
          { id: 'file', title: isChinese ? '系统文件' : 'Choose File' },
        ]}
        cancelLabel={isChinese ? '取消' : 'Cancel'}
        onOpenChange={setAttachmentActionsOpen}
        onSelect={(id) => {
          if (id === 'library') void pickPhoto(false);
          if (id === 'camera') void pickPhoto(true);
          if (id === 'file') void pickFile();
        }}
        open={attachmentActionsOpen}
...
function ModelToolsDrawer({
  ...
  return (
    <PreviewModal
      onClose={onClose}
      open={open}
      title={isChinese ? '模型与工具' : 'Model & Tools'}
    >
      <IOSActionSheet
        actions={[
        ...
        open={Platform.OS === 'ios' && modelOpen}
