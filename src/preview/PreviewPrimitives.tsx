...
import {
  hasNativeProgress,
  hasNativeSearchBar,
  hasNativeSegmentedControl,
  hasNativeSelection,
  hasNativeSwitch,
  HermesProgressView,
  HermesSearchBarView,
  HermesSegmentedControlView,
  HermesSelectionView,
  HermesSwitchView,
...
export function PreviewToggle({
  ...
  if (Platform.OS === 'ios' && hasNativeSwitch) {
    return (
      <HermesSwitchView
        accessibilityLabel={t(accessibilityLabel)}
        disabled={disabled}
        offTintColor={tokens.colors.muted}
        onTintColor={tokens.colors.primary}
        onValueChange={(event) => onChange(event.nativeEvent.value)}
...
export function PreviewProgress({
  ...
  if (Platform.OS === 'ios' && hasNativeProgress) {
    return (
      <HermesProgressView
        progress={progress}
        progressTintColor={color ?? tokens.colors.primary}
        style={styles.nativeProgress}
        trackHeight={6}
        trackTintColor={tokens.colors.muted}
      />
    );
  }
  return (
...
export function PreviewSegmented<T extends string>({
  ...
  if (Platform.OS === 'ios' && hasNativeSegmentedControl) {
    return (
      <HermesSegmentedControlView
        backgroundColorValue={tokens.colors.muted}
        fontName={font}
        fontSize={rootSize * 0.72}
        onValueChange={(event) => {
        ...
        selectedTextColor={tokens.colors.primaryForeground}
        style={[
        ...
        values={options.map((option) => t(option.label))}
      />
    );
  }
  return (
...
export function PreviewModal({
  ...
  return (
    <Modal
      allowSwipeDismissal={iosSheet}
      animationType={iosSheet ? 'slide' : 'fade'}
      onRequestClose={onClose}
      presentationStyle={iosSheet ? 'pageSheet' : 'overFullScreen'}
      statusBarTranslucent={!iosSheet}
      transparent={!iosSheet}
      visible={open}
    >
      <View
        style={[
          iosSheet ? styles.modalSheetRoot : styles.modalRoot,
          iosSheet ? { backgroundColor: opaque(tokens.colors.background) } : null,
        ]}
      >
        {!iosSheet ? (
          <>
