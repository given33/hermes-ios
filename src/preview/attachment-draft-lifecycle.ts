...
export function isUriInsideDirectory(uri: string, directoryUri: string): boolean {
  try {
    ...
    const directory = new URL(directoryUri.endsWith('/') ? directoryUri : `${directoryUri}/`);
    if (candidate.protocol !== directory.protocol || candidate.host !== directory.host) return false;
    const rootPath = directory.pathname.endsWith('/')
      ? directory.pathname
      : `${directory.pathname}/`;
    return candidate.pathname.startsWith(rootPath) && candidate.pathname.length > rootPath.length;
  } catch {
    return false;
  }
}
