...
function renderRoute({
  ...
  if (['plugins', 'mcp', 'channels', 'webhooks'].includes(routeId)) {
    ...
    return <>{(routeId === 'mcp' || routeId === 'webhooks') ? <CreateBar busy={busy} chinese={chinese} draft={draft} onChange={setDraft} onSubmit={() => { void send(HERMES_SWIFTUI_ROUTE_ACTIONS.integrationCreate, { name: draft, fields: routeId === 'mcp' ? { url: draft } : {} }).then(() => setDraft('')); }} placeholder={routeId === 'mcp' ? 'MCP URL' : (chinese ? 'Webhook 名称' : 'Webhook name')} /> : null}<PreviewCard title={routeTitle(routeId, locale)}>{items.map((item) => <PreviewRow key={item.id} style={styles.row}><View style={styles.grow}><PreviewText>{item.name || item.id}</PreviewText><PreviewText variant="muted">{item.detail || ''}</PreviewText></View>{routeId === 'mcp' && item.catalogEntry ? <NativeButton disabled={busy || (item.catalogRequiredEnv?.length ?? 0) > 0} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.integrationCreate, { name: item.name, fields: { catalogName: item.id, enable: 'true' } })} size="sm">{chinese ? '安装' : 'Install'}</NativeButton> : <NativeButton disabled={busy} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.integrationToggle, { id: item.id, enabled: item.enabled !== true })} size="sm">{item.enabled === false ? (chinese ? '启用' : 'Enable') : (chinese ? '停用' : 'Disable')}</NativeButton>}{routeId === 'mcp' && item.canTest ? <NativeButton disabled={busy} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.mcpTest, { id: item.id })} size="sm">{chinese ? '测试' : 'Test'}</NativeButton> : null}{routeId === 'channels' ? <NativeButton disabled={busy} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.integrationUpdate, { id: item.id, fields: parseJsonRecord(item.configuration || '{}') || { enabled: item.enabled !== false } })} size="sm">{chinese ? '保存配置' : 'Save config'}</NativeButton> : null}{(routeId === 'mcp' || routeId === 'webhooks') && !item.catalogEntry ? <NativeButton disabled={busy} destructive onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.integrationDelete, { id: item.id })} size="icon">×</NativeButton> : null}</PreviewRow>)}</PreviewCard></>;
  }
  if (routeId === 'profiles' || routeId === 'profile-new' || routeId === 'bots') {
    const profiles = Array.isArray(snapshot.profiles) ? snapshot.profiles : [];
    return <><CreateBar busy={busy} chinese={chinese} draft={draft} onChange={setDraft} onSubmit={() => { void send(HERMES_SWIFTUI_ROUTE_ACTIONS.profileCreate, { route: routeId, name: draft }).then(() => setDraft('')); }} placeholder={chinese ? '新 Profile 名称' : 'New profile name'} /><PreviewCard title={routeTitle(routeId, locale)}>{profiles.map((item) => { const soul = soulDrafts[item.id] ?? item.soul ?? item.detail ?? ''; return <PreviewRow key={item.id} style={styles.row}><View style={styles.grow}><PreviewText>{item.name || item.id}</PreviewText><TextInput multiline onChangeText={(value) => setSoulDrafts((current) => ({ ...current, [item.id]: value }))} placeholder={chinese ? 'SOUL.md 内容' : 'SOUL.md content'} placeholderTextColor="#8b929c" style={styles.input} value={soul} /><PreviewText variant="muted">{item.model || ''}</PreviewText></View><PreviewBadge tone={item.active ? 'success' : 'outline'}>{item.active ? (chinese ? '当前' : 'Active') : (chinese ? '可用' : 'Available')}</PreviewBadge><NativeButton disabled={busy || item.active === true} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.profileActivate, { route: routeId, id: item.id })} size="sm">{chinese ? '使用' : 'Use'}</NativeButton><NativeButton disabled={busy || !soul.trim()} onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.profileUpdate, { route: 'profiles', id: item.id, detail: soul })} size="sm">{chinese ? '保存灵魂' : 'Save soul'}</NativeButton><NativeButton disabled={busy || item.active === true} destructive onPress={() => void send(HERMES_SWIFTUI_ROUTE_ACTIONS.profileDelete, { route: routeId, id: item.id })} size="icon">×</NativeButton></PreviewRow>; })}</PreviewCard></>;
  }
