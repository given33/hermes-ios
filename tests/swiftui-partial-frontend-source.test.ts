...
test('SwiftUI management pages expose the server write operations', () => {
  ...
  const kanbanActions = read('src/app/route-actions/kanban.ts');
  ...
  assert.match(kanbanActions, /api\.createKanbanTask/);
  assert.match(kanbanActions, /api\.updateKanbanTask/);
  assert.match(kanbanActions, /api\.dispatchKanban/);
  assert.match(kanbanActions, /api\.reassignKanbanTask/);
  assert.match(kanbanActions, /api\.reclaimKanbanTask/);
  assert.match(kanbanActions, /api\.specifyKanbanTask/);
  assert.match(kanbanActions, /api\.decomposeKanbanTask/);
  assert.match(kanbanActions, /api\.inspectKanbanRun/);
  assert.match(kanbanActions, /api\.terminateKanbanRun/);
  assert.match(kanbanActions, /api\.getKanbanTaskLog/);
  assert.match(kanbanActions, /api\.switchKanbanBoard/);
  assert.match(routes, /HermesKanbanTaskDetailSheet/);
  assert.match(routes, /detailJSON: data\.kanbanDetailJSON/);
  assert.match(routes, /expectedTaskID: card\.id/);
  assert.match(routes, /\.kanbanTaskOpen/);
  assert.match(routes, /\.kanbanDispatch/);
  assert.match(routes, /\.kanbanReassign/);
  assert.match(routes, /\.kanbanReclaim/);
  assert.match(routes, /\.kanbanSpecify/);
  assert.match(routes, /\.kanbanDecompose/);
  assert.match(routes, /\.kanbanRunInspect/);
  assert.match(routes, /\.kanbanRunTerminate/);
  assert.match(routes, /\.kanbanTaskLog/);
  assert.match(routes, /\.kanbanBoardSwitch/);
  assert.match(routes, /\.confirmationDialog\(/);
