...
test('native auth root is integrated without an AppState foreground re-prompt listener', () => {
  ...
  assert.match(loginSource, /#170d02/i);
  assert.match(loginSource, /#ffac02/i);
  assert.match(loginSource, /#ffffff/i);
  assert.match(loginSource, /#ff6b6b/i);
  ...
  assert.match(loginSource, /#0053fd/i);
  assert.match(loginSource, /LOGIN_DITHER_SIZE\s*=\s*3/);
  assert.match(loginSource, /LOGIN_DITHER_OPACITY\s*=\s*0\.04/);
  assert.match(loginSource, /LOGIN_GLOW_OPACITY\s*=\s*0\.06/);
  assert.match(loginSource, /LOGIN_GLOW_STOP\s*=\s*'55%'/);
  assert.match(loginSource, /LOGIN_GLOW_RADIUS_X\s*=\s*'70\.710678%'/);
  assert.match(loginSource, /LOGIN_GLOW_RADIUS_Y\s*=\s*'141\.421356%'/);
  ...
  assert.match(loginSource, /Animated\.Value\(6\)/);
  assert.match(loginSource, /duration:\s*600/);
  assert.match(loginSource, /Easing\.bezier\(0,\s*0,\s*0\.58,\s*1\)/);
  ...
  assert.match(loginSource, /CARD_SHADOW_SPREAD\s*=\s*20/);
  assert.match(loginSource, /CARD_SHADOW_BLUR_SIGMA\s*=\s*30/);
  assert.match(loginSource, /CARD_SHADOW_OFFSET_Y\s*=\s*24/);
  assert.match(loginSource, /CARD_SHADOW_OPACITY\s*=\s*0\.6/);
