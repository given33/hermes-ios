...
test('authenticated native content is reachable through the native ThemeProvider', () => {
  ...
  assert.match(appSource, /<ThemeProvider client=\{client\}[\s\S]*preferenceNamespace=\{ownerScope\}>/);
  assert.match(appSource, /Hermes authenticated content/);
  assert.match(
    appSource,
    /<FrontendPreviewApp[\s\S]{0,180}cacheOwner=\{ownerScope\}[\s\S]{0,80}client=\{client\}[\s\S]{0,80}notificationTarget=\{notificationTarget\}/,
  );
