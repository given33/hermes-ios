...
private enum HermesIntentQueueCipher {
  ...
  private static func key() throws -> SymmetricKey {
    if let existing = readKey(), existing.count == 32 {
      // A build that predates mirroring may have left only one spelling;
      // heal the other side on every read so a later process without the
      // group entitlement cannot silently start a divergent key.
  ...
  private static func mirrorPlainItem(bytes: Data) {
    ...
    // A stale mirror may hold divergent bytes generated by an earlier
    // no-entitlement build; the group item is canonical, so overwrite it.
    var query = plain
