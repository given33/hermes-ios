...
这是当前定制 Hermes WebUI 的完全原生 React Native 客户端，不包含 WebView 或 WKWebView。WebUI 源代码是 UI、字体、颜色、尺寸、渲染、特效、动画和功能行为的唯一规格；原生端只替换底层实现机制，不削弱视觉或交互结果。

## 已实现

- 所有者账号首次注册与密码登录、Keychain 令牌保护和 Face ID 快速解锁。
...
- Hermes 地址：环境变量 `EXPO_PUBLIC_HERMES_URL`，默认 `https://daxueshenmai.top`。
- GitHub 仓库：环境变量 `EXPO_PUBLIC_GITHUB_REPOSITORY`，默认 `given33/hermes-ios`。
- bundle identifier：`app.sunstone1029.fig1171`。
- 最低系统版本：iOS 18.0。
